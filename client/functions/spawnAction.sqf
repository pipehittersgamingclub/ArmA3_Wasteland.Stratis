...
spawnActionHandle = (_this select 1) spawn
{
	...
	if (isNil "playerData_resetPos") then
	{
		...
		_baseMoney = ["A3W_startingMoney", 15000] call getPublicVar;
		switch (_donatorLevel) do
		{
			case 1: { player setVariable ["cmoney", _baseMoney, true]; [MF_ITEMS_CANNED_FOOD, 1] call mf_inventory_add; [MF_ITEMS_WATER, 1] call mf_inventory_add; };
			case 2: { player setVariable ["cmoney", _baseMoney, true]; };
			case 3: { player setVariable ["cmoney", _baseMoney, true]; };
			case 4: { player setVariable ["cmoney", _baseMoney, true]; };
			case 5: { player setVariable ["cmoney", _baseMoney + 15000, true];	};
			case 6: { player setVariable ["cmoney", _baseMoney + 15000, true];	};			
